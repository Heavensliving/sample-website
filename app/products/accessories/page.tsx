...
import React, { useState, useEffect } from 'react';
import Image from 'next/image';
import { motion, Variants, useScroll, useTransform, AnimatePresence } from 'framer-motion'; // <-- ADDED AnimatePresence
import Navbar from '@/components/Navbar'; // Import Navbar
import Footer from '@/components/Footer'; // Import Footer
import VarahaHeroSection from './VarahaHeroSection';
...
import AkUpgradeKitDetail from './AkUpgradeKitDetail';


// --- Advanced Animation Variants (No changes) ---
const heroVariants: Variants = {
  hidden: { opacity: 0, scale: 1.15 },
  visible: { 
    opacity: 1, 
...
// ParticlesBackground component (No changes)
const ParticlesBackground = () => {
  const particles = Array.from({ length: 30 });
  const [isClient, setIsClient] = useState(false);
  useEffect(() => {
      ...
      {isClient && particles.map((_, i) => {
        ...
        return (
          <motion.div
            ...
            animate={{ x: animateX, y: animateY, opacity: [0.2, 0.5, 0.2] }}
            transition={{
              duration: Math.random() * 20 + 10,
              repeat: Infinity,
              ease: "linear"
...
// Tactical Grid Lines Background (No changes)
const TacticalGrid = () => (
  <div className="absolute inset-0 opacity-[0.03] pointer-events-none">
    <svg width="100%" height="100%">
      <defs>
...
const AccessoriesPage: React.FC = () => {
  ...
  const [showAkDetail, setShowAkDetail] = useState(false);
  ...
  return (
    <div className="min-h-screen bg-black text-white flex flex-col overflow-hidden">
      ...
      <main className="flex-grow flex flex-col relative z-10">
        ...
        <section className="relative w-full h-[50vh] md:h-[65vh] lg:h-[80vh] overflow-hidden">
          {/* ... (all hero code remains the same) ... */}
          <motion.div
            className="absolute inset-0"
            style={{ scale: heroScale, opacity: heroOpacity }}
          >
        ...
        <section className="container mx-auto px-4 py-16 md:py-24">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
            ...
            {/* AK Upgrade Kit --- MODIFIED --- */}
            <motion.div 
              className="relative bg-zinc-900/50 rounded-lg overflow-hidden group border border-zinc-800 cursor-pointer" // <-- ADDED CURSOR
              initial={{ opacity: 0, y: 50 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true, amount: 0.3 }}
              transition={{ duration: 0.6, delay: 0.1 }}
              onClick={() => setShowAkDetail(true)} // <-- ADDED ONCLICK
            >
              <Image
                src="/Accessories/Ak.png"
                alt="AK Upgrade Kit"
                width={700}
                height={400}
                layout="responsive"
      ...
      <AnimatePresence>
        {showAkDetail && (
          <AkUpgradeKitDetail onClose={() => setShowAkDetail(false)} />
        )}
      </AnimatePresence>
