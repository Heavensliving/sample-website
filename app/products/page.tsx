...
const ProductsPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white flex flex-col">
      ...
      {/* No change needed here, this setup is fine */}
      <main className="flex-grow flex justify-center p-6 lg:p-8 xl:p-12">
        <motion.div 
          className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 max-w-screen-xl mx-auto w-full mt-40"
          variants={containerVariants}
        ...
        >
          {productCategories.map((category, index) => {
            ...
            // This logic is already responsive and correct.
            const verticalOffset = isEven ? 'mt-0' : 'lg:mt-8';
            ...
            return (
              <motion.a
                key={category.id}
                href={category.link}
                // 🎯 RESPONSIVE FIX 1: Change aspect ratio
                // Use aspect-[5/4] (wider) on mobile, then switch to aspect-[3/4] (taller) on lg screens.
                className={`relative aspect-[5/4] lg:aspect-[3/4] overflow-hidden group cursor-pointer ${verticalOffset}`}
                style={{
                  clipPath: 'polygon(0 0, 90% 0, 100% 10%, 100% 100%, 10% 100%, 0 90%)',
                }}
                variants={cardVariants}
              ...
              >
                ...
                {/* 🎯 RESPONSIVE FIX 2: Change text alignment */}
                {/* Mobile (base): All text is at the 'bottom-0' and 'items-end'.
                  Large (lg): For '!isEven' cards, override to 'lg:top-0', 'lg:items-start', 
                              and 'lg:bottom-auto' to remove the 'bottom-0' default.
                */}
                <div className={`absolute inset-x-0 bottom-0 p-6 flex items-end ${!isEven ? 'lg:bottom-auto lg:top-0 lg:items-start' : ''}`}>
                  <motion.div
                    // 🎯 RESPONSIVE FIX 3: Unify animation
                    // Always animate from the bottom (y: 20) for a consistent mobile feel.
                    // The old logic (isEven ? 20 : -20) caused text to animate from the wrong place on mobile.
                  ...
                  >
                    ...
                    {/* 🎯 RESPONSIVE FIX 4: Adjust text size ramp */}
                    {/* Start smaller on mobile (text-lg) and scale up. */}
                    <h2 className={`text-lg sm:text-xl lg:text-2xl font-bold uppercase tracking-wider text-white ${isVaraha ? 'group-hover:text-blue-400' : 'group-hover:text-red-500'} transition-colors duration-300 drop-shadow-lg`}>
                      {category.title}
                    </h2>
