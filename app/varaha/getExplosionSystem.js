...
function getExplosionSystem(params) {
  ...
  function _AddParticles(timeElapsed) {
    ...
    for (let i = 0; i < n; i += 1) {
      ...
      velocity.normalize().multiplyScalar(Math.random() * 5.0 + 2.0); // Speed

      _particles.push({
        position: emitter.position.clone(), // Start at the center
        size: (Math.random() * 0.5 + 0.5) * maxSize * 2.0, // Make particles bigger
        ...
        velocity: velocity, // Use our new outward velocity
      });
    }
  }
  ...
  // This new function creates a one-time burst
  function trigger(numParticles) {
    // We pass '1.0' as timeElapsed to spawn all particles at once
    // based on the original function's rate logic
    gdfsghk = numParticles / rate;
    _AddParticles(1.0 / rate); // Pass a small delta
  }

  function update(timeElapsed) {
    // We REMOVE _AddParticles(timeElapsed);
    _UpdateParticles(timeElapsed);
    _UpdateGeometry();
  }
  return { update, trigger, points: _points };
}

export { getExplosionSystem, getLinearSpline };
