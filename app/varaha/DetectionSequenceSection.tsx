...
// Optimized constants based on device
const STARS_COUNT = isMobile ? 300 : isTablet ? 600 : 1000;
const PARTICLE_COUNT = isMobile ? 80 : isTablet ? 140 : 200;
const FIRE_SPRITES = isMobile ? 8 : isTablet ? 10 : 12;

// Mobile-optimized positions (closer to camera, more compact)
const DRONE_START_POS = isMobile
  ? new THREE.Vector3(-15, 12, 0)
  : new THREE.Vector3(-20, 15, 0);
const DRONE_TRACK_POS = isMobile
  ? new THREE.Vector3(-2, 8, 0)
  : new THREE.Vector3(-2, 10, 0);
const SENSOR_1_POS = isMobile
  ? new THREE.Vector3(12, 4, -10)
  : new THREE.Vector3(15, 5, -14);
const SENSOR_2_POS = isMobile
  ? new THREE.Vector3(12, 4, -18)
  : new THREE.Vector3(15, 5, -24);
const EFFECTOR_POS = isMobile
  ? new THREE.Vector3(6, 4, 0)
  : new THREE.Vector3(8, 5, 0);
const CONTROL_ROOM_POS = isMobile
  ? new THREE.Vector3(8, 4, 2)
  : new THREE.Vector3(10, 5, 2);

// Landscape scale
const LANDSCAPE_SCALE = isMobile ? 90 : isTablet ? 105 : 120;
...
const Loader: React.FC = () => (
  <Html center>
    <div className="flex flex-col items-center">
      ...
      <div className="mt-3 sm:mt-4 text-white font-mono text-sm sm:text-lg tracking-wider">
        LOADING 3D SEQUENCE
      </div>
    </div>
  </Html>
  ...
  const copiedScene = useMemo(() => {
    ...
    clone.traverse((child: any) => {
      if (child.isMesh) {
        ...
        if (child.material && (child.material as any).map) {
          (child.material as any).map.anisotropy = isMobile ? 2 : 8;
        }
      }
    });
    return clone;
...
// ---------------- AnimatedFireSprite (throttled) ----------------
const AnimatedFireSprite: React.FC<{ texture: THREE.Texture }> = memo(
  ({ texture }) => {
    const spriteRef = useRef<THREE.Sprite | null>(null);
    const startTime = useRef<number>(Date.now());
    const duration = useMemo(() => Math.random() * 800 + 500, []);
...
AnimatedFireSprite.displayName = "AnimatedFireSprite";

// ---------------- Dome ----------------
type DomeProps = JSX.IntrinsicElements["group"] & { stage: AnimationStage };
const Dome: React.FC<DomeProps> = memo(({ stage, ...props }) => {
  const { scene } = useGLTF("/dome_sensor/result.gltf") as any;
  const copiedScene = useMemo(() => scene.clone(), [scene]);
  const groupRef = useRef<THREE.Group | null>(null);
  ...
  useFrame((state, delta) => {
    ...
    if (stage === "tracking" || stage === "firing") {
      ...
      temp.position.copy(g.position);
      temp.lookAt(DRONE_TRACK_POS);
      g.quaternion.slerp(temp.quaternion, delta * 3);
    } else {
      g.rotation.y = Math.sin(state.clock.elapsedTime * 0.5) * 0.3;
    }
  });
...
// ---------------- Drone ----------------
const Drone: React.FC<{ stage: AnimationStage }> = memo(({ stage }) => {
  const { scene } = useGLTF("/drone-3d-model/drone-3d-model.gltf") as any;
  const ref = useRef<THREE.Group | null>(null);
  const propellerRefs = useRef<THREE.Object3D[]>([]);
  ...
  useFrame((state, delta) => {
    ...
    if (stage === "approaching") {
      r.position.lerp(DRONE_TRACK_POS, delta * 0.8);
      r.rotation.z = Math.sin(state.clock.elapsedTime * 2) * 0.1;
    } else if (stage === "tracking" || stage === "firing") {
      r.position.x =
        DRONE_TRACK_POS.x + Math.sin(state.clock.elapsedTime * 2) * 0.2;
...
// ---------------- SensorScan ----------------
const SensorScan: React.FC<{ position: THREE.Vector3 }> = memo(
  ({ position }) => {
    const ref = useRef<THREE.Mesh | null>(null);
    useFrame(({ clock }) => {
      const r = ref.current;
...
SensorScan.displayName = "SensorScan";
...
TrackingPulse.displayName = "TrackingPulse";
...
FireTrail.displayName = "FireTrail";

// ---------------- Shockwave ----------------
const Shockwave: React.FC = memo(() => {
  const ref = useRef<THREE.Mesh | null>(null);
  const startTime = useRef<number>(Date.now());
  const duration = 500;
  const targetScale = 8;
...
// ---------------- RealisticFireExplosion ----------------
const RealisticFireExplosion: React.FC = memo(() => {
  const fireTexture = useLoader(
    THREE.TextureLoader,
    "/fire.png"
  ) as THREE.Texture;
  const sprites = useMemo(
    () =>
      Array.from({ length: FIRE_SPRITES }).map((_, i) => (
        <AnimatedFireSprite key={i} texture={fireTexture} />
      )),
    [fireTexture]
  );
...
RealisticFireExplosion.displayName = "RealisticFireExplosion";

// ---------------- Explosion ----------------
const Explosion: React.FC<{ position: THREE.Vector3 }> = memo(
  ({ position }) => (
    <group position={position}>
      <RealisticFireExplosion />
      <Shockwave />
...
// ---------------- CameraRig ----------------
const CameraRig: React.FC<{ stage: AnimationStage; mode: CameraMode }> = memo(
  ({ stage, mode }) => {
    const { camera } = useThree();
    const vec = useMemo(() => new THREE.Vector3(), []);
    const lookAtTarget = useMemo(() => new THREE.Vector3(0, 2, 0), []);
    ...
    useFrame(() => {
      ...
      let targetPos: [number, number, number] = [0, 25, 80];
      let lookAtPos: [number, number, number] = [0, 2, 0];
      const terrainOffset = isMobile ? 12 : 15;

      if (stage === "approaching") {
        targetPos = isMobile
          ? [-25, 10 + terrainOffset, 40]
          : [-30, 12 + terrainOffset, 50];
        lookAtPos = [DRONE_TRACK_POS.x, DRONE_TRACK_POS.y, DRONE_TRACK_POS.z];
      } else if (stage === "tracking") {
        targetPos = isMobile
          ? [20, 10 + terrainOffset, 20]
          : [25, 12 + terrainOffset, 25];
        lookAtPos = [DRONE_TRACK_POS.x, DRONE_TRACK_POS.y, DRONE_TRACK_POS.z];
      } else if (stage === "firing") {
        targetPos = isMobile
          ? [25, 7 + terrainOffset, 12]
          : [30, 8 + terrainOffset, 15];
        lookAtPos = [DRONE_TRACK_POS.x, DRONE_TRACK_POS.y, DRONE_TRACK_POS.z];
      } else if (stage === "destroyed") {
        targetPos = isMobile
          ? [-5, 10 + terrainOffset, 20]
          : [-5, 12 + terrainOffset, 25];
        lookAtPos = [DRONE_TRACK_POS.x, DRONE_TRACK_POS.y, DRONE_TRACK_POS.z];
      } else {
        targetPos = isMobile
          ? [0, 20 + terrainOffset, 65]
          : [0, 25 + terrainOffset, 80];
      }
      ...
      lookAtTarget.lerp(
        vec.set(lookAtPos[0], lookAtPos[1], lookAtPos[2]),
        0.03
      );
      camera.lookAt(lookAtTarget);
    });
...
const DetectionSequenceSection: React.FC = memo(() => {
  ...
  // Responsive DPR and camera settings
  const dpr: [number, number] = isMobile
    ? [0.75, 1]
    : isTablet
    ? [1, 1.5]
    : [1, 2];
  const cameraFov = isMobile ? 60 : isTablet ? 55 : 50;
  const cameraDistance = isMobile ? 65 : isTablet ? 75 : 80;

  return (
    <section
      ref={ref as unknown as React.RefObject<HTMLElement>}
      className="relative w-full min-h-screen bg-gradient-to-b from-gray-900 via-black to-gray-900 text-white overflow-hidden py-12 sm:py-16 px-4 sm:px-6 lg:px-8"
    >
      <motion.h2
        className="text-2xl sm:text-3xl md:text-4xl font-black uppercase text-center mb-6 sm:mb-8 text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-cyan-400 to-blue-600"
        initial={{ opacity: 0, y: 20 }}
        animate={isInView ? { opacity: 1, y: 0 } : {}}
        transition={{ duration: 0.8 }}
      >
      ...
      <motion.div
        className="relative w-full h-[60vh] sm:h-[70vh] lg:h-[75vh] max-w-7xl mx-auto rounded-lg overflow-hidden shadow-2xl shadow-blue-500/20"
        initial={{ opacity: 0, scale: 0.9 }}
        animate={isInView ? { opacity: 1, scale: 1 } : {}}
        transition={{ duration: 0.8, delay: 0.3 }}
      >
        <Canvas
          shadows
          camera={{ position: [0, 30, cameraDistance], fov: cameraFov }}
          dpr={dpr}
          gl={{
            antialias: !isMobile,
            alpha: false,
